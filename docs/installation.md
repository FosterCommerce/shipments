...
- **Order statuses to ignore**: orders in these Commerce statuses are automatically marked as not requiring shipping. Any existing shipments on them are disabled and they drop off the Attention page. Adding a handle triggers a one-time retroactive sweep of orders currently in that status. Default: empty.
